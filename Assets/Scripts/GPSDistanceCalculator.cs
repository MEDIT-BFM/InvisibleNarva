...
public class GPSDistanceCalculator : MonoBehaviour {

    float DegToRad(float deg)
    {
        float temp;
        temp = (float)((deg * Math.PI) / 180.0f);
        temp = Mathf.Tan(temp);
        return temp;
    }
    ...
    {
        float a_long_r, a_lat_r, p_long_r, p_lat_r, dist_x, dist_y;
        ...
        totalDistance = Final_distance(dist_x, dist_y);
